...
public class HEED extends ClusteringProtocol{
	...
	public void setRadiusAndNeighbors() {
		...
		ArrayList<CallableRadiusAndNeighbours> al = new ArrayList<CallableRadiusAndNeighbours>();
		for(int i=0;i<this.wsn.wsnNodeList.size();i++){
			HEEDNODE temp = (HEEDNODE)this.wsn.wsnNodeList.get(i);
			
			al.add(new CallableRadiusAndNeighbours(temp, wsn));
		}
		try {
			exS.invokeAll(al);
	...
	private class CallableRadiusAndNeighbours implements Callable<Object>{
		private Node n;
		private WSN wsn;
		public CallableRadiusAndNeighbours(Node n, WSN wsn){
			this.n=n;
			this.wsn=wsn;
		}
		@Override
		public Object call() throws Exception {
			...
			System.out.println("Adding Neighbor"+n.id+":"+n.printNeighborList());
			return null;
		}
	}
			...
			while(iterator.hasNext()){
				...
				if(node.CHpro_previous < 1){ 
					...
					if(node.SCH.size() != 0){							//someone proposed as CH. Can be the node itself
						...
						node.sendFinalCH();
					}// diventare CH anche se prob <> da 1

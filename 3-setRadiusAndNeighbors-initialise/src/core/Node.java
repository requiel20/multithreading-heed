...
public class Node extends NodeCircleShape implements Comparable<Object>{
	 ...
	 Object sentMessageLock = new Object();
	...
	//RECEIVED MSG
	protected volatile long totalReceveivedMessages=0;		//total amount of messages received by the node totalReceveivedMessages=totalReceivedControlMessages + totalReceivedIntraTraffic + totalReceivedInterTraffic
	protected volatile long totalReceivedControlMessages=0;	//messages received during election, cluster formation and routing establishment
	...
	//DINAMIC INFO
	 public SortedSet<Node> neighborAtMaxRadiusAlive =Collections.synchronizedSortedSet(new TreeSet<Node>());
	  public SortedSet<Node> neighborAtRadiusAlive = Collections.synchronizedSortedSet(new TreeSet<Node>());
	...
	public void setNeighbor(){
		Node dest;
		double distance;
		// raggio picolo interno cluster grande tra un cluster e altro
		//figli di cluster raggio piccolo e comunica con altri CH a raggio grande 
		//se non ci diventa deve stare a raggio piccolo
		neighborAtRadiusAlive		= Collections.synchronizedSortedSet(new  TreeSet<Node>());
		neighborAtMaxRadiusAlive	= Collections.synchronizedSortedSet(new  TreeSet<Node>());
		
		Iterator<Node> iterator=wsn.aliveNodeList.iterator();
		//Per ogni nodo si itera per tutti i nodi
		while(iterator.hasNext()){
			...
			if(dest.id!=this.id){
				...
				if(distance<this.radius){						//All nodes within  radius are included in both sets
					...
					synchronized (this.sentMessageLock) {
						dest.totalReceivedControlMessages++;
					}
